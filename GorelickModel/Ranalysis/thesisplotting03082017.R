...
A = ggplot(DNmelt) + geom_line(aes(x = variable, y = value, group = Sim, color = group), 
                           size = 0.6, alpha = 0.8) + 
  facet_grid(FacetSet ~ .) + guides(col = T) +
  ...
  guides(col = guide_legend(title.position = "top", direction = "horizontal", label.position = "right", 
                            ncol = 1, keywidth = 0.4, keyheight = 0.2, default.unit = "inch",
                            title = "Formulation")) +
  theme(legend.position = "left", 
        ...
        plot.margin = unit(c(0.4,0.2,0.5,0.2), unit ="in"),
...
# for (i in 1:length(maxChars))  {
#   A <- A + annotation_custom(
#     grob = grid::textGrob(label = maxChars[i], vjust = 0, gp = grid::gpar(cex = 1.5)),
#     ymin = 1,      # Vertical position of the textGrob
#     ymax = 1,
#     xmin = minChars[i],         # Note: The grobs are positioned outside the plot area
#     xmax = minChars[i])
# }    
# 
...
ggsave(paste(figurepath,"ParallelFacets.png",sep = ""), 
       width = 8, height = 7.6, units = "in")
...
ggsave(paste(figurepath, "IBTfigure", ".png", sep = ""), 
