...
Suby2 = Sub2[which(Sub2$Year < endyear & Sub2$Year > startyear),]

Suby0 = Suby0[with(Suby0, order(Year, Week)),]
Suby1 = Suby1[with(Suby1, order(Year, Week)),]
Suby2 = Suby2[with(Suby2, order(Year, Week)),]

Suby0$TimeseriesWeek = c(1:nrow(Suby0))
Suby1$TimeseriesWeek = c(1:nrow(Suby1))
Suby2$TimeseriesWeek = c(1:nrow(Suby2))

maxrel1 = max(Suby1$RRR)
maxrel2 = max(Suby2$RRR)
maxbb   = max(Suby2$BBD)

Suby2$BBD = Suby2$BBD/max(Suby2$BBD)*0.30
Suby1$RRR = Suby1$RRR/max(Suby1$RRR)*0.30
Suby2$RRR = Suby2$RRR/max(Suby2$RRR)*0.30
...
plotR = ggplot() +
  geom_line(data = Suby0, aes(x = TimeseriesWeek, y = Rstor, colour = "Storage (Without Releases)")) +
  geom_line(data = Suby1, aes(x = TimeseriesWeek, y = Rstor, colour = "Storage (Spot Releases)")) +
  geom_line(data = Suby2, aes(x = TimeseriesWeek, y = Rstor, colour = "Storage (Option Releases)")) +
  geom_line(data = Suby0, aes(x = TimeseriesWeek, y = RaleighTargetStorageFraction, colour = "ROF Guide Curve")) +
  geom_bar(data  = Suby2, aes(x = TimeseriesWeek, y = RRR, fill = RRR*maxrel2/0.3), stat = "identity") +
  scale_colour_discrete(guide = FALSE) +
  scale_fill_continuous(name = "Releases (top) and buybacks (bottom)\n(MGW)") +
...
plotD = ggplot() +
  geom_line(data = Suby0, aes(x = TimeseriesWeek, y = Dstor, colour = "Storage (Without Releases)")) +
  geom_line(data = Suby1, aes(x = TimeseriesWeek, y = Dstor, colour = "Storage (Spot Releases)")) +
  geom_line(data = Suby2, aes(x = TimeseriesWeek, y = Dstor, colour = "Storage (Option Releases)")) +
  geom_line(data = Suby0, aes(x = TimeseriesWeek, y = DurhamTargetStorageFraction, colour = "ROF Guide Curve")) +
  geom_bar(data  = Suby2, aes(x = TimeseriesWeek, y = BBD, fill = BBD*maxbb/0.3), stat = "identity") +
...
for (FORMULATION in 0:2)
{
  ...
  Tdata = aggregate(x = Tdata,    by = list(Tdataannual$Realization), FUN = max, na.rm = T)
...
TTDdensplot = ggplot(alldata) + 
  geom_density(aes(TTD0, y = ..count.., fill = "0", colour = "0"), alpha = trans) +
  geom_density(aes(TTD1, y = ..count.., fill = "1", colour = "1"), alpha = trans) +
  geom_density(aes(TTD2, y = ..count.., fill = "2", colour = "2"), alpha = trans) +
#  scale_fill_discrete(name = "Formulation") +
  guides(colour = F, fill = F) + labs(title = "Durham") + 
  xlim(c(0,1000)) +
  theme(legend.position = c(0.87, 0.91),
        legend.background = element_rect(fill = "grey90", colour = "black")) + 
  xlab("Total realization transfers (MG)") + ylab("Count") 
#  geom_text(data = data.frame(), aes(2000/10, 0.0015), label = "Durham", size = 9, hjust = 0, vjust = 0)

RestRdensplot = ggplot(alldata) + 
  geom_density(aes(restR0, y = ..count.., fill = "0", colour = "0"), alpha = trans) +
  geom_density(aes(restR1, y = ..count.., fill = "1", colour = "1"), alpha = trans) +
  geom_density(aes(restR2, y = ..count.., fill = "2", colour = "2"), alpha = trans) +
#  scale_fill_discrete(name = "Formulation") +
...
RestDdensplot = ggplot(alldata) + 
  geom_density(aes(x = restD0, y = ..count.., fill = "0", colour = "0"), alpha = trans) +
  geom_density(aes(x = restD1, y = ..count.., fill = "1", colour = "1"), alpha = trans) +
  geom_density(aes(x = restD2, y = ..count.., fill = "2", colour = "2"), alpha = trans) +
#  scale_fill_discrete(name = "Formulation") + 
  guides(colour = F, fill = F) + 
#  xlim(c(0,7)) +
  theme(legend.position = c(0.87, 0.91),
        legend.background = element_rect(fill = "grey90", colour = "black")) + 
  xlab("Average weeks under restriction annually") + ylab("Count") 
...
for (RANK in startrank:(startrank+3))
{
  ALLTdata = read.csv(paste(datafolder, "ALLtransferData", RANK, "_", FORMULATION , "_", RDMnum, ".csv", sep = ""), header = T)
  ObjData  = read.csv(paste(datafolder, "simulationOutput", RANK, "_", FORMULATION , "_", RDMnum, ".csv", sep = ""), header = F)
...
colnames(ParallelPlotData4) = c("group", objnames)
write.csv(ParallelPlotData4, paste(figurepath, "ParallelPlotDatabyInfrastructureBuildDay.csv", sep = ""), row.names = F)
